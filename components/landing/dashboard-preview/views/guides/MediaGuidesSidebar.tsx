...
export function MediaGuidesSidebar({ guides, mediaType }: MediaGuidesSidebarProps) {
  ...
  const color = isVideos ? "text-blue-600" : "text-purple-600";
  const bgColor = isVideos ? "bg-blue-50" : "bg-purple-50";
  const borderColor = isVideos ? "border-blue-200" : "border-purple-200";
  ...
  return (
    <div className="space-y-4">
      {/* Media Summary */}
      <div className={`bg-gradient-to-br ${bgColor} to-white rounded-xl border ${borderColor} p-4`}>
        <h3 className="text-sm font-medium text-gray-900 mb-3">{label} Library</h3>
        <div className="text-center mb-3">
          ...
          <p className="text-[10px] text-gray-500">Total {label}</p>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-white rounded-lg p-2.5 border border-gray-200 text-center">
            <p className="text-sm font-bold text-blue-600">{completedCount}</p>
            <p className="text-[9px] text-gray-500">Completed</p>
          </div>
          <div className="bg-white rounded-lg p-2.5 border border-gray-200 text-center">
            <p className="text-sm font-bold text-amber-600">{inProgressCount}</p>
      ...
      {topSources.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-sm font-medium text-gray-900">By Source</h3>
          </div>
