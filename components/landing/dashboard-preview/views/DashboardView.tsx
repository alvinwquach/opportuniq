...
export function DashboardView() {
  ...
  return (
    <div className={`flex flex-col h-full ${panelBg}`}>
      ...
      {/* Recall alert */}
      <div className={`mx-4 mt-2 mb-0 rounded-lg border p-3 flex items-start gap-2.5 ${dark ? "border-amber-500/20 bg-amber-500/5" : "border-amber-100 bg-amber-50"}`}>
        <div className="w-6 h-6 rounded-md bg-amber-100 flex items-center justify-center flex-shrink-0">
          <IoShieldOutline className="w-3.5 h-3.5 text-amber-600" />
        </div>
        <div className="flex-1 min-w-0">
          <p className={`text-xs font-semibold ${dark ? "text-amber-400" : "text-amber-700"}`}>CPSC Recall Match</p>
          <p className={`text-[10px] mt-0.5 ${dark ? "text-amber-500" : "text-amber-600"}`}>Your Carrier 59SC5A furnace has an active recall for heat exchanger defect</p>
        </div>
        <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium flex-shrink-0 bg-amber-100 text-amber-700`}>CPSC</span>
      </div>

      {/* Scrollable content area */}
      <div className="flex-1 min-h-0 scrollbar-auto-hide p-4 space-y-4">

        {/* ── Row 1: Open Issues + Budget/Savings ── */}
        <div className="grid grid-cols-3 gap-4">

          {/* Open Issues (col-span-2) */}
          <div className={`col-span-2 rounded-xl border ${b} ${panelBg}`}>
          ...
          {/* Budget + Savings (col-span-1) */}
          <div className={`col-span-1 rounded-xl border ${b} ${panelBg} p-4`}>
            ...
            <div className={`border-t ${b} mt-4 pt-3`}>
              ...
              <div className="mt-2 flex items-end gap-0.5 h-8">
                {savingsOverTime.map((d, i) => {
                  const maxSavings = savingsOverTime[savingsOverTime.length - 1].savings;
                  const height = Math.round((d.savings / maxSavings) * 100);
                  return (
                    <div key={d.month} className="flex-1 flex flex-col items-center gap-0.5">
                      <div
                        ...
                        style={{ height: `${height}%` }}
                      />
                      <span className={`text-[7px] ${labelCls}`}>{d.month}</span>
                    </div>
                  );
                })}
              </div>
        ...
        {/* ── Row 2: Guides + Weather + Quick Actions ── */}
        <div className="grid grid-cols-3 gap-4">

          {/* In Progress Guides */}
          <div className={`rounded-xl border ${b} ${panelBg}`}>
            <div className={`flex items-center justify-between px-4 py-2.5 border-b ${b}`}>
              <div className="flex items-center gap-2">
          ...
          {/* Weather */}
          <div className={`rounded-xl border ${b} ${panelBg} p-4`}>
        ...
        {/* ── Row 3: Activity + Nearby ── */}
        <div className="grid grid-cols-2 gap-4">

          {/* Activity */}
          <div className={`rounded-xl border ${b} ${panelBg}`}>
            <div className={`flex items-center gap-2 px-4 py-2.5 border-b ${b}`}>
