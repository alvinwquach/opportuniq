  ...
  return (
    <div className="w-40 flex-shrink-0 h-full flex flex-col border-r border-white/[0.06] bg-[#0d0d0d]">
      <div className="flex items-center justify-between p-2 border-b border-white/[0.06]">
        ...
        <button onClick={onClose} className="p-1 text-white/60 hover:text-white rounded">
      ...
      <div className="p-2 border-b border-white/[0.06]">
        <div className="relative">
          <IoSearch className="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-white/60" />
          <input
            type="text"
            placeholder="Search..."
            value={searchQuery}
            onChange={(e) => onSearchChange(e.target.value)}
            className="w-full pl-7 pr-2 py-1 text-[11px] bg-[#171717] border border-white/[0.06] rounded-md placeholder:text-white/50 text-white focus:outline-none focus:border-emerald-500"
          />
        </div>
      </div>
              ...
              <div className="flex items-start gap-1.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1 mt-0.5">
                    <span className="text-[9px] text-white/60">{issue.date}</span>
