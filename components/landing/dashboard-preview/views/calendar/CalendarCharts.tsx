...
export function CalendarCharts() {
  return (
    <div className="grid md:grid-cols-3 gap-4">
      {/* Event Distribution */}
      <div className="bg-[#1a1a1a] rounded-xl border border-[#2a2a2a] p-4 overflow-visible">
        <h3 className="text-sm font-medium text-white mb-1">Event Distribution</h3>
        <p className="text-xs text-[#666] mb-3">By type this month</p>
        ...
        <div className="flex justify-center gap-4 mt-2">
          {eventTypeData.map((item) => (
            <div key={item.name} className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-[#888]">{item.name.replace(' Projects', '')}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Weekly Activity */}
      <div className="bg-[#1a1a1a] rounded-xl border border-[#2a2a2a] p-4 overflow-visible">
        <h3 className="text-sm font-medium text-white mb-1">Weekly Activity</h3>
        <p className="text-xs text-[#666] mb-3">Events vs expenses</p>
        <div className="h-[160px] overflow-visible">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={weeklyActivityData} barGap={2}>
              ...
              <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 10, fill: '#666' }} axisLine={false} tickLine={false} hide />
              <Tooltip
                wrapperStyle={{ zIndex: 1000 }}
                contentStyle={{
                  backgroundColor: '#1a1a1a',
              ...
              />
              <Bar yAxisId="left" dataKey="events" fill="#3ECF8E" radius={[4, 4, 0, 0]} name="Events" barSize={16} />
      ...
      <div className="bg-[#1a1a1a] rounded-xl border border-[#2a2a2a] p-4">
        <h3 className="text-sm font-medium text-white mb-1">Completion Rate</h3>
        <p className="text-xs text-[#666] mb-3">Scheduled vs completed</p>
        ...
        <div className="flex justify-center gap-6 mt-2">
          <div className="flex items-center gap-1.5">
            <div className="w-2 h-2 rounded-full bg-emerald-500" />
            <span className="text-[10px] text-[#888]">Scheduled</span>
          </div>
          <div className="flex items-center gap-1.5">
