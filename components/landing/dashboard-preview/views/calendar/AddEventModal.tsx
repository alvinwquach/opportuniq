...
export function AddEventModal({ isOpen, newEvent, onClose, onSave, onChange }: AddEventModalProps) {
  ...
  const getTypeStyles = (typeId: string, color: string) => {
    ...
    const colorMap: Record<string, { bg: string; border: string; text: string }> = {
      emerald: { bg: 'rgba(62, 207, 142, 0.2)', border: 'rgba(62, 207, 142, 0.3)', text: '#2563EB' },
  ...
  return createPortal(
    <div className="fixed inset-0 z-[10000] flex items-center justify-center">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto border border-gray-200">
        ...
        <div className="p-5 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-500 mb-1.5">Event Title</label>
            <input
              type="text"
              value={newEvent.title}
              onChange={(e) => onChange({ ...newEvent, title: e.target.value })}
              placeholder="e.g., Fix kitchen faucet"
              className="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-lg text-gray-900 placeholder:text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-500 mb-1.5">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-500 mb-1.5">Event Type</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-500 mb-1.5">Location (optional)</label>
            <div className="relative">
              <IoLocationOutline className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-600" />
              <input
                type="text"
                value={newEvent.location}
                onChange={(e) => onChange({ ...newEvent, location: e.target.value })}
                placeholder="e.g., Kitchen, Basement"
                className="w-full pl-9 pr-3 py-2 text-sm bg-white border border-gray-200 rounded-lg text-gray-900 placeholder:text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-500 mb-1.5">Recurring</label>
            <div className="flex items-center gap-3">
              ...
              {newEvent.isRecurring && (
                <select
                  ...
                  className="flex-1 px-3 py-2 text-sm bg-white border border-gray-200 rounded-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
          ...
          <div>
            <label className="block text-sm font-medium text-gray-500 mb-1.5">Reminder</label>
            <select
          ...
          <div>
            <label className="block text-sm font-medium text-gray-500 mb-1.5">Notes (optional)</label>
            <textarea
              value={newEvent.notes}
              onChange={(e) => onChange({ ...newEvent, notes: e.target.value })}
              placeholder="Any additional details..."
              rows={3}
              className="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-lg text-gray-900 placeholder:text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
            />
          </div>
        </div>
