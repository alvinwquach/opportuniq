...
interface GroupUpdatedEmailProps {
  memberName: string;
  groupName: string;
  updatedBy: string;
  changes: string[];
...
export const GroupUpdatedEmail = ({
  memberName = 'there',
  groupName = 'My Group',
  updatedBy = 'Someone',
  changes = ['Group name updated'],
  groupUrl = 'https://opportuniq.app/dashboard/groups/abc123',
}: GroupUpdatedEmailProps) => {
  return (
    <Html>
      <Head />
      ...
      <Body style={main}>
        <Container style={container}>
          ...
          <Section style={contentCard}>
            <Heading style={heroTitle}>Group Settings Updated</Heading>
            <Text style={heroSubtitle}>
              Hi {memberName}, <strong>{updatedBy}</strong> made changes to your group.
            </Text>
            ...
            <Section style={changesSection}>
              ...
              {changes.map((change, index) => (
                <Text key={index} style={changeItem}>• {change}</Text>
              ))}
            </Section>
...
export default GroupUpdatedEmail;
