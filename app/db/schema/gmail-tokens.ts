/**
 * GMAIL TOKENS SCHEMA - Stores OAuth tokens for Gmail API access
 *
 * SECURITY:
 * - Tokens are encrypted at rest by Supabase
 * - Access tokens expire after 1 hour, refresh tokens are long-lived
 * - Users can revoke access at any time
 * - Only gmail.send scope is requested (no read access)
 */

import { pgTable, uuid, text, timestamp, boolean } from "drizzle-orm/pg-core";
import { users } from "./users";

/**
 * GMAIL TOKENS TABLE
 *
 * Stores OAuth tokens for sending emails via Gmail API.
 * One-to-one relationship with users (each user can have one Gmail connection).
 */
export const gmailTokens = pgTable("gmail_tokens", {
  // Primary key - same as user ID for 1:1 relationship
  userId: uuid("user_id")
    .primaryKey()
    .references(() => users.id, { onDelete: "cascade" }),

  // Gmail email address connected (may differ from account email)
  gmailAddress: text("gmail_address").notNull(),

  // OAuth access token - expires after ~1 hour
  accessToken: text("access_token").notNull(),

  // OAuth refresh token - long-lived, used to get new access tokens
  refreshToken: text("refresh_token").notNull(),

  // When the access token expires (UTC)
  expiresAt: timestamp("expires_at").notNull(),

  // OAuth scopes granted by the user
  // Should include: https://www.googleapis.com/auth/gmail.send
  scopes: text("scopes").notNull(),

  // Whether the connection is active (user hasn't revoked)
  isActive: boolean("is_active").default(true).notNull(),

  // When the Gmail account was connected
  connectedAt: timestamp("connected_at").defaultNow().notNull(),

  // Last time tokens were refreshed
  lastRefreshedAt: timestamp("last_refreshed_at"),

  // Last time an email was sent via this connection
  lastUsedAt: timestamp("last_used_at"),
});

// Type exports
export type GmailToken = typeof gmailTokens.$inferSelect;
export type NewGmailToken = typeof gmailTokens.$inferInsert;
