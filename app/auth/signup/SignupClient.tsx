"use client";

import { useState } from "react";
import { createClient } from "@/lib/supabase/client";
import { Button } from "@/components/ui/button";
import { IoHome, IoPeople, IoCheckmarkCircle } from "react-icons/io5";
import Link from "next/link";

interface SignupClientProps {
  inviteToken?: string | null;
  groupName?: string | null;
}

export function SignupClient({ inviteToken, groupName }: SignupClientProps) {
  const [loading, setLoading] = useState<string | null>(null);
  const [error, setError] = useState("");

  const handleOAuthSignup = async (provider: "github" | "google" | "linkedin_oidc") => {
    setLoading(provider);
    setError("");

    const supabase = createClient();

    // Build redirect URL with invitation token if present
    // IMPORTANT: Always use the canonical domain (without www) to match Supabase config
    const canonicalOrigin = typeof window !== "undefined" && window.location.hostname === "www.opportuniq.app"
      ? "https://opportuniq.app"
      : window.location.origin;

    let redirectUrl = `${canonicalOrigin}/auth/callback`;
    if (inviteToken) {
      redirectUrl += `?token=${inviteToken}`;
    }

    const { error } = await supabase.auth.signInWithOAuth({
      provider,
      options: {
        redirectTo: redirectUrl,
      },
    });

    if (error) {
      setError(error.message);
      setLoading(null);
    }
    // Note: If successful, user will be redirected, so no need to setLoading(null)
  };

  return (
    <div className="min-h-screen grid lg:grid-cols-2">
      <div className="hidden lg:flex flex-col justify-between p-12 bg-slate-900 text-white">
        <div>
          <Link href="/" className="flex items-center gap-2.5">
            <div className="h-10 w-10 rounded-xl bg-emerald-600 flex items-center justify-center">
              <IoHome className="h-5 w-5 text-white" />
            </div>
            <span className="font-display font-bold text-xl">
              OpportunIQ
            </span>
          </Link>
        </div>
        <div className="space-y-6">
          <div>
            <h1 className="text-3xl font-display font-bold mb-3 leading-tight">
              Make smarter repair decisions
            </h1>
            <p className="text-slate-400 text-base leading-relaxed">
              Stop overpaying. Stop guessing. Start making confident decisions backed by research and data.
            </p>
          </div>
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              <div className="mt-1 shrink-0">
                <IoCheckmarkCircle className="h-5 w-5 text-emerald-500" />
              </div>
              <div>
                <h3 className="font-semibold text-white mb-1">Instant Diagnosis</h3>
                <p className="text-slate-400 text-sm">Upload a photo, video clip, or describe the issue. Get repair options and cost estimates.</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <div className="mt-1 shrink-0">
                <IoCheckmarkCircle className="h-5 w-5 text-emerald-500" />
              </div>
              <div>
                <h3 className="font-semibold text-white mb-1">Collaborate with Your Group</h3>
                <p className="text-slate-400 text-sm">Share budgets, track expenses, and make decisions together with family, roommates, or tenants.</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <div className="mt-1 shrink-0">
                <IoCheckmarkCircle className="h-5 w-5 text-emerald-500" />
              </div>
              <div>
                <h3 className="font-semibold text-white mb-1">Track Everything</h3>
                <p className="text-slate-400 text-sm">Maintenance history, budgets, contractor ratings, and decision outcomes—all in one place.</p>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="flex items-center justify-center p-6 lg:p-12 bg-white dark:bg-slate-950">
        <div className="w-full max-w-md">
          {groupName && (
            <div className="mb-6 p-4 rounded-lg bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800">
              <div className="flex items-center gap-2 text-emerald-700 dark:text-emerald-400">
                <IoPeople className="h-5 w-5" />
                <p className="font-medium">
                  You&apos;ve been invited to join <span className="font-bold">{groupName}</span>
                </p>
              </div>
            </div>
          )}
          <div className="mb-8">
            <h2 className="text-3xl font-display font-bold text-slate-900 dark:text-white mb-2">
              Create account
            </h2>
            <p className="text-slate-600 dark:text-slate-400">
              Get started with OpportunIQ
            </p>
          </div>
          <div className="space-y-3">
            <Button
              onClick={() => handleOAuthSignup("github")}
              disabled={loading !== null}
              variant="outline"
              className="w-full h-12 text-base border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-900"
              size="lg"
            >
              {loading === "github" ? (
                <div className="flex items-center gap-2">
                  <div className="h-4 w-4 border-2 border-slate-400 border-t-transparent rounded-full animate-spin" />
                  <span>Creating account...</span>
                </div>
              ) : (
                <>
                  <svg className="h-5 w-5 mr-3" fill="currentColor" viewBox="0 0 24 24">
                    <path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" />
                  </svg>
                  Continue with GitHub
                </>
              )}
            </Button>
            <Button
              onClick={() => handleOAuthSignup("google")}
              disabled={loading !== null}
              variant="outline"
              className="w-full h-12 text-base border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-900"
              size="lg"
            >
              {loading === "google" ? (
                <div className="flex items-center gap-2">
                  <div className="h-4 w-4 border-2 border-slate-400 border-t-transparent rounded-full animate-spin" />
                  <span>Creating account...</span>
                </div>
              ) : (
                <>
                  <svg className="h-5 w-5 mr-3" viewBox="0 0 24 24">
                    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                    <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                    <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                  </svg>
                  Continue with Google
                </>
              )}
            </Button>
            <Button
              onClick={() => handleOAuthSignup("linkedin_oidc")}
              disabled={loading !== null}
              variant="outline"
              className="w-full h-12 text-base border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-900"
              size="lg"
            >
              {loading === "linkedin_oidc" ? (
                <div className="flex items-center gap-2">
                  <div className="h-4 w-4 border-2 border-slate-400 border-t-transparent rounded-full animate-spin" />
                  <span>Creating account...</span>
                </div>
              ) : (
                <>
                  </>
              )}
            </Button>
          </div>
          {error && (
            <div className="mt-4 p-4 rounded-lg bg-red-50 dark:bg-red-950/50 border border-red-200 dark:border-red-800">
              <p className="text-sm text-red-600 dark:text-red-400">{error}</p>
            </div>
          )}
          <div className="mt-8 text-center">
            <p className="text-sm text-slate-600 dark:text-slate-400">
              Already have an account?{" "}
              <Link href="/auth/login" className="text-emerald-600 hover:text-emerald-500 font-medium">
                Sign in
              </Link>
            </p>
          </div>
          <p className="text-xs text-slate-500 dark:text-slate-400 text-center mt-6">
            By signing up, you agree to our{" "}
            <Link href="/terms-of-service" className="underline hover:text-slate-700 dark:hover:text-slate-300">
              Terms
            </Link>{" "}
            and{" "}
            <Link href="/privacy-policy" className="underline hover:text-slate-700 dark:hover:text-slate-300">
              Privacy Policy
            </Link>
            .
          </p>
        </div>
      </div>
    </div>
  );
}
